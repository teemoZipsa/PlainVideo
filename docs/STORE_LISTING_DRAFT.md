...
Status: Submission 1 is live in Microsoft Store; version 0.2.0 bilingual copy, release notes, screenshot captions, and inherited media are prepared for Submission 2
...
Click to play or pause, double-click for fullscreen, preview a position by hovering over the seek bar, and use the arrow keys to seek or change volume. Toggle subtitles directly from the transient playback bar or choose a track from the context menu. PlainVideo automatically discovers matching external subtitles, resumes partially watched files, and remembers volume and a safe physical window position across monitors.
...
Seek with clean thumbnail previews, resume partially watched files, save playback screenshots, and inspect richer media details without interrupting playback. Subtitle toggling, volume feedback, window controls, click response, and drag behavior are now more consistent and immediate.
...
클릭으로 재생하거나 일시정지하고, 더블 클릭으로 전체화면을 전환하며, 탐색바에 마우스를 올려 해당 시점의 썸네일을 미리 볼 수 있습니다. 방향키로 탐색과 볼륨을 조절하고, 잠깐 나타나는 재생 바에서 자막을 바로 켜고 끄거나 우클릭 메뉴에서 트랙을 고를 수 있습니다. 같은 이름의 외부 자막을 자동으로 찾고, 일부 재생한 파일의 위치와 볼륨, 모니터별 안전한 물리 픽셀 창 위치를 기억합니다.
...
탐색바 썸네일 미리보기, 이어보기, 재생 화면 스크린샷 저장, 더 풍부한 미디어 정보를 추가했습니다. 자막 토글과 볼륨 피드백, 창 버튼, 클릭 반응, 창 이동 동작도 더 일관되고 즉각적으로 다듬었습니다.
...
Suggested captions for Submission 2:
...
with the release record. Submission 2 retains the four rights-cleared
screenshots per locale and adds localized captions without interrupting the
user's desktop for another foreground capture run.
